function test_check() {
	...
	style_values.doClick('terms-accept');
	label_maker.changeSelect('usapost-delivery', 3);
	...
	if (container_size == 'large') {
		style_values.showErrorsCheck('CHECK', check_snapshot_large_expected);
	} else if (container_size == 'medium') {
		style_values.showErrorsCheck('CHECK', check_snapshot_medium_expected);
	} else {
		style_values.showErrorsCheck('CHECK', check_snapshot_small_expected);
	}
